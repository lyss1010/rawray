...
Vector3 Plastica::Shade(const Ray& ray, const HitInfo& hit, const Scene& scene) const {
    ...
    for (light_it=lights.begin(); light_it != lights.end(); ++light_it) {
        ...
        // Diffuse component
        const float intensity = std::max( 0.0f, 1.0f/falloff * light->GetWattage() / math::PI );
