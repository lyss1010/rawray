...
void Worley::Noise1D(float at, long maxOrder, float *F, float(*delta), unsigned long *ID) {
    float x2, mx2;
    float newAt;
    long intAt, i;
    ...
    for (i = 0; i < maxOrder; i++) F[i] = 999999.9f;
    ...
    {
        F[i] = sqrt(F[i]) * (1.0f / DENSITY_ADJUSTMENT);
        delta[i] *= (1.0f / DENSITY_ADJUSTMENT);
    }
}


void Worley::Noise2D(float at[2], long maxOrder, float *F, float(*delta)[2], unsigned long *ID) {
    float x2, y2, mx2, my2;
    float newAt[2];
    long intAt[2], i;
    ...
    {
        F[i] = sqrt(F[i]) * (1.0f / DENSITY_ADJUSTMENT);
        delta[i][0] *= (1.0f / DENSITY_ADJUSTMENT);
        delta[i][1] *= (1.0f / DENSITY_ADJUSTMENT);
    }
}

void Worley::Noise3D(float at[3], long maxOrder, float *F, float(*delta)[3], unsigned long *ID) {
    ...
    for (i = 0; i < maxOrder; i++)
    {
        ...
        delta[i][0] *= (1.0f / DENSITY_ADJUSTMENT);
        delta[i][1] *= (1.0f / DENSITY_ADJUSTMENT);
        delta[i][2] *= (1.0f / DENSITY_ADJUSTMENT);
    }
}

void Worley::AddSamples(long xi, long maxOrder, float at, float *F, float(*delta), unsigned long *ID) {
    float dx, fx, d2;
...
void Worley::AddSamples(long xi, long yi, long maxOrder, float at[2], float *F, float(*delta)[2], unsigned long *ID) {
...
void Worley::AddSamples(long xi, long yi, long zi, long maxOrder, float at[3], float *F, float(*delta)[3], unsigned long *ID) {
...
const int Worley::poissonCount[256] =
{
